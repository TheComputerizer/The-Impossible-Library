package mods.thecomputerizer.theimpossiblelibrary.shared.v20.m6.client.gui;
...
public class ScreenWrapper1_20_6 extends Screen implements Wrapped<ScreenAPI> {
    ...
    public ScreenWrapper1_20_6(ScreenAPI wrapped) {
        super(wrapped.getTitle().getAsComponent());
        this.wrapped = wrapped;
    }
    
    @Override public boolean charTyped(char c, int mods) {
        if(StringUtil.isAllowedChatCharacter(c) && this.wrapped.onCharTyped(c)) return true;
    ...
    @Override public boolean mouseScrolled(double mouseX, double mouseY, double deltaX, double deltaY) {
        if(Objects.nonNull(this.wrapped) && deltaY!=0d) {
            if(deltaY>=1d) {
                if(this.wrapped.scrollUp(MathHelper.clamp(deltaY,-1d,1d))) return true;
            } else if(this.wrapped.scrollDown(MathHelper.clamp(deltaY,-1d,1d))) return true;
        }
