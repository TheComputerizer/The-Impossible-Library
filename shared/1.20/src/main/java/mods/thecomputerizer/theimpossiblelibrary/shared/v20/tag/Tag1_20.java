...
public class Tag1_20 implements TagAPI {
    ...
    @Override public <T> BaseTagAPI<T> getWrapped(T tag) {
        if(tag instanceof CompoundTag) return GenericUtils.cast(new CompoundTag1_20(tag));
        if(tag instanceof ListTag) return GenericUtils.cast(new ListTag1_20(tag));
        if(tag instanceof NumericTag) return GenericUtils.cast(new PrimitiveTag1_20(tag));
        if(tag instanceof StringTag) return GenericUtils.cast(new StringTag1_20(tag));
        return null;
    }
