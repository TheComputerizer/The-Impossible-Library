...
public class SoundHelper1_20 implements SoundHelperAPI {
    ...
    @Override public void setCategoryVolume(String name, float volume) {
        ...
        if(Objects.nonNull(category)) {
            Options options = Minecraft.getInstance().options;
            if(Objects.nonNull(options)) options.getSoundSourceOptionInstance(category).set((double)volume);
