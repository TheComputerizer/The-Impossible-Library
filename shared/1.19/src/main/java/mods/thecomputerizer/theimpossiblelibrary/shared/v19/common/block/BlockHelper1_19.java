...
public class BlockHelper1_19 implements BlockHelperAPI {
    ...
    @Override public <V extends Comparable<V>> BlockPropertyAPI<?,V> createProperty(String name, V defVal) {
        if(defVal instanceof Boolean) return new BlockProperty1_19<>(BooleanProperty.create(name));
        if(defVal instanceof Enum<?>) return new BlockProperty1_19<>(createPropertyEnum(name,defVal.getClass()));
        if(defVal instanceof Number)
            return new BlockProperty1_19<>(IntegerProperty.create(name,0,((Number)defVal).intValue()));
    ...
    @Override public <P> BlockPropertyAPI<?,?> getAsProperty(P property) {
        if(property instanceof Property<?>) return new BlockProperty1_19<>(property);
