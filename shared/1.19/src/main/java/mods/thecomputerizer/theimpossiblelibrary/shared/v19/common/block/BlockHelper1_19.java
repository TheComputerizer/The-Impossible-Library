...
public class BlockHelper1_19 implements BlockHelperAPI {
    
    @SuppressWarnings("unchecked") @Override public <V extends Comparable<V>> BlockPropertyAPI<?,V> createProperty(String name, V defVal) {
        if(defVal instanceof Boolean) return new BlockProperty1_19<>((Property<V>)BooleanProperty.create(name));
        if(defVal instanceof Enum<?>) return new BlockProperty1_19<>((Property<V>)createPropertyEnum(name, defVal.getClass()));
        if(defVal instanceof Number) return new BlockProperty1_19<>((Property<V>)IntegerProperty.create(name, 0, ((Number)defVal).intValue()));
    ...
    @Override public <P> BlockPropertyAPI<?,?> getAsProperty(P property) {
        if(property instanceof Property<?>) return new BlockProperty1_19<>((Property<?>)property);
        TILRef.logError("Object {} is not an instance of {}!",property,Property.class);
        return null;
    }
    
    @Override public Material1_19 getMaterialByName(String name) {
        if(StringUtils.isBlank(name)) return new Material1_19(WOOD);
        Field field = ReflectionHelper.getField(Material.class, name.toUpperCase());
        if(Objects.nonNull(field)) return new Material1_19(ReflectionHelper.getFieldInstance(null, field));
        return new Material1_19(AIR);
    }
    
    @Override public MaterialColor1_19 getMaterialColorByName(String name) {
        if(StringUtils.isBlank(name)) return new MaterialColor1_19(GRASS);
        Field field = ReflectionHelper.getField(MaterialColor.class, name.toUpperCase());
        if(Objects.nonNull(field))
            return new MaterialColor1_19((MaterialColor)ReflectionHelper.getFieldInstance(null,field));
        return new MaterialColor1_19(GRASS);
    }
}
