...
public class Tag1_19 implements TagAPI {
    ...
    @Override public <T> BaseTagAPI<T> getWrapped(T tag) {
        if(tag instanceof CompoundTag) return GenericUtils.cast(new CompoundTag1_19(tag));
        if(tag instanceof ListTag) return GenericUtils.cast(new ListTag1_19(tag));
        if(tag instanceof NumericTag) return GenericUtils.cast(new PrimitiveTag1_19(tag));
        if(tag instanceof StringTag) return GenericUtils.cast(new StringTag1_19(tag));
        return null;
    }
