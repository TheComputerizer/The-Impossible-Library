...
public class RegistryHandlerForge1_18_2 extends RegistryHandler1_18_2 {
    ...
    private <V extends IForgeRegistryEntry<V>> RegistryForge1_18_2<V> getRegistry(
            Set<? super Registry1_18_2<?>> registries, IForgeRegistry<V> forgeRegistry, String name,
            Class<?> type) {
        ResourceLocationAPI<?> key = WrapperHelper.wrapResourceLocation(new ResourceLocation(name));
        RegistryForge1_18_2<V> registry = new RegistryForge1_18_2<>(forgeRegistry,key,type);
        registries.add(registry);
        return registry;
    }
    
    @SuppressWarnings("SameParameterValue")
    private <V> Registry1_18_2<V> getVanillaRegistry(Set<? super Registry1_18_2<?>> registries,
            Registry<V> vanillaRegistry, String name, Class<?> type) {
        ResourceLocationAPI<?> key = WrapperHelper.wrapResourceLocation(new ResourceLocation(name));
        RegistryVanilla1_18_2<V> registry = new RegistryVanilla1_18_2<>(vanillaRegistry,key,type);
        registries.add(registry);
        return registry;
    }
