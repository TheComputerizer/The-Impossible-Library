...
public class DataHelper {
    ...
    public static void initGlobal() {
        try {
            writeExplanation(FileHelper.generateNestedFile("./impossible_data/what_is_this_folder.txt",false));
        } catch (IOException e) {
            LogHelper.logInternal(Level.ERROR, "There was an error generating the data folder or explanation file");
            GLOBAL_LOAD_FAILED = true;
        }
    }

    private static void writeExplanation(File file) throws IOException {
        if(Objects.nonNull(file)) FileHelper.writeLinesToFile(file, EXPLANATION,false);
    ...
    public static CompoundTag getWorldData(String modid, @Nonnull String worldName) {
        try {
            ...
            LogHelper.logInternal(Level.ERROR, "Unable to read mod data for modid {} in world {}", modid, worldName, ex);
        }
        return null;
    }
    ...
    public static void writeGlobalData(CompoundTag data, String modid) throws IOException {
        ...
        else LogHelper.logInternal(Level.WARN, "There was a problem when initializing global data for The Impossible Library so data for {} could not be written",modid);
    }
    ...
    public static CompoundTag getGlobalData(String modid, boolean createIfAbsent) throws IOException {
        ...
        LogHelper.logInternal(Level.WARN, "There was a problem when initializing global data for The Impossible Library so data for {} could not be retrieved",modid);
        return null;
    }
    ...
    private static void writeFileData(CompoundTag data, File directory, String modid) throws IOException {
        ...
        dataFile = FileHelper.generateNestedFile(dataFile,true);
        if(Objects.nonNull(dataFile)) NbtIo.write(data, dataFile);
        else LogHelper.logInternal(Level.ERROR,"Could not write data for {} due to an error in creating the file",modid);
    }
    ...
    private static CompoundTag getFileData(File directory, String modid, boolean createIfAbsent) throws IOException {
        ...
        if (createIfAbsent) {
            dataFile = FileHelper.generateNestedFile(dataFile,false);
