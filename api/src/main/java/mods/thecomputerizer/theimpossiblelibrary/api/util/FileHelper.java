...
public class FileHelper {
    ...
    public static File generateNestedFile(File file, boolean overrideExisting) {
        ...
        if(!canProceed && overrideExisting) {
            ...
            if(!canProceed) LogHelper.logInternal(Level.ERROR,"Failed to delete existing file at path {}",file.getAbsolutePath());
        }
        if(canProceed) {
            ...
            try {
                ...
                LogHelper.logInternal(Level.ERROR,"Failed to create file at path {}",file.getAbsolutePath(), e);
                return null;
            }
            if (!success) {
                LogHelper.logInternal(Level.ERROR,"Failed to create file at path {}",file.getAbsolutePath());
                return null;
            }
        }
        return file;
    ...
    public static void writeLineToFile(File file, String text, boolean append) {
        try {
            ...
            LogHelper.logInternal(Level.ERROR,"Failed to write line {} to file {}",text,file.getAbsolutePath(),e);
        }
    }
    ...
    public static void writeLinesToFile(File file, List<String> text, boolean append) {
        try {
            ...
            LogHelper.logInternal(Level.ERROR,"Failed to write lines {} to file {}",text,file.getAbsolutePath(),e);
        }
    }
