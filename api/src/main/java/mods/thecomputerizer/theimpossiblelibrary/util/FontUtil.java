...
/**
 * The implementation of FontRenderer here is a lot different from the 1.12.2 version, so I will be leaving this alone
 * for 1.16.5+ until I can make sense of it
 */
public class FontUtil {
    ...
    public static void renderChar(char c, Vector2f pos, FontRenderer font, Vector4f colors) {
        if(c==160 || c==' ') return;
     ...
     //respectively
    public static Vector4f bufferAndGetCharUV(char c, FontRenderer font) {
    ...
    public static Vector4f getCharUV(char c, FontRenderer font) {
        if(c==160 || c==' ') return new Vector4f(0f,1f,0f,1f);
        int i = ASCII_CHARS.indexOf(c);
    ...
    private static void bufferAsciiTex(FontRenderer font) {
        Minecraft.getInstance().getTextureManager().bind(font.locationFontTexture);
    }
    private static Vector4f getAsciiUV(int c, float width) {
        float u = (float)(c%16*8);
        float v = (float)(c/16*8);
        return new Vector4f(u/128f,(u+width-1f)/128f,v/128f,(v+7.99f)/128f);
    }
    private static void renderAsciiChar(int c, Vector2f pos, float width, FontRenderer font, Vector4f colors) {
        bufferAsciiTex(font);
        drawAsciiChar(false,pos,getAsciiUV(c,width),width - 0.01f,colors);
    }
    private static void drawAsciiChar(boolean triangles, Vector2f pos, Vector4f uv, float width, Vector4f colors) {
        if(triangles) {
    ...
    private static Vector4f getUnicodeUV(char c, int glyphWidth) {
        if(glyphWidth==0) return new Vector4f(0f,1f,0f,1f);
        ...
        return new Vector4f(u/256f,(u+width)/256f,v/256f,(v+15.98f)/256f);
    }
    private static void renderUnicodeChar(char c, Vector2f pos, FontRenderer font, Vector4f colors) {
        int i = font.glyphWidth[c] & 255;
        if (i == 0) return;
    ...
    private static void drawUnicodeChar(boolean triangles, Vector2f pos, Vector4f uv, float width, Vector4f colors) {
        if(triangles) {
            GlStateManager.glBegin(GL11.GL_TRIANGLE_STRIP);
            GuiUtil.tupleColor(colors);
            GlStateManager.glTexCoord2f(uv.x, uv.z);
    ...
    public static int convertChatFormatting(ChatFormatting format, int a) {
        ...
        return switch (format) {
            case DARK_RED -> GuiUtil.makeRGBAInt(170, 0, 0, a);
            case RED -> GuiUtil.makeRGBAInt(255, 85, 85, a);
            case GOLD -> GuiUtil.makeRGBAInt(255, 170, 0, a);
            case YELLOW -> GuiUtil.makeRGBAInt(255, 255, 85, a);
            case DARK_GREEN -> GuiUtil.makeRGBAInt(0, 170, 0, a);
            case GREEN -> GuiUtil.makeRGBAInt(85, 255, 85, a);
            case AQUA -> GuiUtil.makeRGBAInt(85, 255, 255, a);
            case DARK_AQUA -> GuiUtil.makeRGBAInt(0, 170, 170, a);
            case DARK_BLUE -> GuiUtil.makeRGBAInt(0, 0, 170, a);
            case BLUE -> GuiUtil.makeRGBAInt(85, 85, 255, a);
            case LIGHT_PURPLE -> GuiUtil.makeRGBAInt(255, 85, 255, a);
            case DARK_PURPLE -> GuiUtil.makeRGBAInt(170, 0, 170, a);
            case GRAY -> GuiUtil.makeRGBAInt(170, 170, 170, a);
            case DARK_GRAY -> GuiUtil.makeRGBAInt(85, 85, 85, a);
            case BLACK -> GuiUtil.makeRGBAInt(0, 0, 0, a);
            default -> GuiUtil.makeRGBAInt(255, 255, 255, a);
        };
    }
}